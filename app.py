...
from knowledge_base import init_knowledge_base, get_all_tips, get_tip_by_id, get_categories
...
def journal(site_id):
    ...
    from database import get_journal_entries_filtered, get_distinct_work_types, get_distinct_responsible
    
    entries = get_journal_entries_filtered(
        site_id, 
        search=search,
        date_from=date_from,
        date_to=date_to,
    ...
    # Получаем уникальные типы работ и ответственных для фильтров
    work_types = get_distinct_work_types(site_id)
    responsible_persons = get_distinct_responsible(site_id)
...
def knowledge_base():
    ...
    search = request.args.get('search', '')
    tips = get_all_tips(category if category else None, search if search else None)
    categories = get_categories()
    return render_template('knowledge.html', tips=tips, categories=categories, 
                         selected_category=category, search=search)

@app.route('/knowledge/<int:tip_id>')
def knowledge_detail(tip_id):
    tip = get_tip_by_id(tip_id)
    if not tip:
        return redirect(url_for('knowledge_base'))
    return render_template('knowledge_detail.html', tip=tip)
